...
def get_system_health(supabase):
    ...
    # Get all articles
    all_articles = supabase.table('3point5percent_articles').select('*').execute()
...
def get_source_performance(supabase):
    ...
    # Get all sources
    sources = supabase.table('3point5percent_sources').select('*').execute()
    
    # Get all articles
    articles = supabase.table('3point5percent_articles').select('source_name, relevance_score').execute()
